...
VAE 使用 512 像素分块、384 像素步长（128 像素重叠）。在同一份 1536×1536 生图中间结果的对照中，默认 256/192 分块产生的周期性色条在 512/384 解码后不再明显；本机此次解码的 PyTorch 分配显存峰值约 2.25GB，整图解码则发生显存不足。这是单样本验证，不保证所有图片均无瑕疵。旧图片不会自动更新，需要重新生成；保存了中间结果的诊断样本可以直接重新解码。
