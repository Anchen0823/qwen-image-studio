...
VAE 输出采用上下文分块：每块保留 256 像素中心，四周各多解码 256 像素上下文，舍弃人工块边缘，保留原图外边界。结果在 CPU 拼接，RGBA 四个通道全部保留。显存不足时仅重试解码，将上下文缩为 128 像素，不重新采样，也不回退到已知存在色条的混合方式。参考图编码仍使用原有分块参数。

此前仅扩大分块到 512/384 的方案在浅色背景仍有周期性色条，提高到 FP32 也不能消除。新的中心裁取方案已用两份 1536×1536 中间结果（浅底中文信息图、人物插画）验证；浅底样本解码约 32 秒，PyTorch 分配显存峰值约 4.17GB。另以 768×768 整图解码作参照，平均误差从约 0.35 降至 0.11（0–255 色阶）。分块解码仍是近似计算，不保证所有图像与整图解码逐像素一致。旧图不会自动更新；未保存中间结果的旧图需要重新生成。

`tests/test_vae_decode.py` 检查中心裁取对矩形、不完整末块、小图、RGBA 和批次的覆盖，以及解码显存不足时的有界重试。需使用安装了 PyTorch 的模型环境运行。
